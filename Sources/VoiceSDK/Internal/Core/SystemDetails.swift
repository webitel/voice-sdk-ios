...
class SystemDetails {
    ...
    private func createUserAgent() -> String {
        let agent = "\(appNameAndVersion()) (\(deviceName()); \(platform()); \(deviceVersion())) \(sdkVersion()) \(CFNetworkVersion()) \(DarwinVersion())"
        let validAgent = agent.removingNonPrintableASCII()
    ...
    private func sdkVersion() -> String {
        let frameworkBundle = Bundle(identifier: "com.webitel.WebitelSdkIos")
        let frameworkVersion = frameworkBundle?.infoDictionary?["CFBundleShortVersionString"] as? String ?? ""
        return "webitel-sdk-ios/\(frameworkVersion)"
    }
